...
fn simulate_fights(player: &Player, monster: &Monster, min_mp: &mut isize, mp: isize, hard: bool) {
    for spell in SPELLS.iter() {
        ...
        if player.is_dead() { continue; }
        if monster.is_dead() { if mp + mp_cost < *min_mp { *min_mp = mp + mp_cost }; continue; }
        ...
        simulate_fights(&player, &monster, min_mp, mp + mp_cost, hard);
    }
}
